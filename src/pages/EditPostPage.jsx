...
import { Edit3, ArrowLeft, Save, AlertCircle, RefreshCw } from 'lucide-react';
  ...
  useEffect(() => {
    const fetchPost = async () => {
      try {
        ...
        const isOwner = user && (user.id === post.userId || user.name === post.author);
        if (!isAdmin && !isOwner) {
          alert('Você não tem permissão para editar postagens de outro autor.');
          navigate(`/posts/${id}`);
          return;
        }
        ...
        setApiError('Não foi possível carregar os dados desta postagem para edição.');
      } finally {
        setLoading(false);
      }
    };
      ...
      try {
        ...
        setApiError(
          err.response?.data?.error ||
            'Não foi possível atualizar a postagem. Verifique suas permissões.'
        );
      } finally {
        setSubmitting(false);
      }
    ...
    return (
      <PageContainer>
        <LoadingContainer>
          ...
          <span>Carregando dados da postagem...</span>
        </LoadingContainer>
      </PageContainer>
    );
  }
